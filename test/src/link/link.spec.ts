import { SingleNode, SingleLink } from './single_link';
import { DoubleNode, DoubleLink } from "./double_link";
...
describe('single', () => {
  ...
  it('link node', () => {
    const data = 'test';
    const root = new SingleNode(data);

    const tailData = 'tail';
    const tail = new SingleNode(tailData);
    root.next = tail;
